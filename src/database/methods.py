...
from database import async_database, sync_database
...
class AsynchronousMethods:
    ...
    async def insert_one(cls, data: dict = None, collection: str = None) -> dict:
        ...
        cursor = await cls.find(filter_dict={"_id": data.get("_id")}, collection=collection)
        results = [document async for document in cursor]
        if len(results) > 0:
            logger.warning(f"_id already exits.")
            return {}
    ...
    async def find(cls, filter_dict: dict = None, collection: str = None, page: int = 1) -> list:
        if not filter_dict:
            results = await cls.db[collection].find().skip((page-1)*settings.MAX_ITEMS_PER_PAGE).limit(settings.MAX_ITEMS_PER_PAGE)
        else:
            results = await cls.db[collection].find(filter_dict).skip((page-1)*settings.MAX_ITEMS_PER_PAGE).limit(settings.MAX_ITEMS_PER_PAGE)
